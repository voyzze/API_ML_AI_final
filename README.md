...
>**一句话**：图像识别技术满足大众在生活中对物体的求知。

>**一分钟**：本应用程序将通过图像识别提供价值，用户把想了解的物体进行拍照识别，接口匹配目标物体并返回相似度最高的结果以及其相关信息。当用户查阅相关信息并觉得匹配有误时可重新识别，第二次的返回结果将按分数排序返回相似度较高的几个结果，用户可从此找到真正想要的答案和认识更多与此目标物体相似的其它物体。至少在用户求知的一刻，图像识别能帮到他们，并满足他们当下的求知欲和认知分享心理。
...
2.  用户身边的亲戚特别是年长者会发图片询问用户关于识别主体的信息
-  * _如何解决？_

     建议用户将图片保存至本地相册后在APP中使用上传识别。     
     
3. 有些原创独特的菜品无法识别，数据库无此菜品信息
- * _如何解决？_

     当识别不到时，建议用户输入菜品中的原材料，搜索引擎结合得出相似的菜品信息。
     
4. 当驱车驶过遇到好看的植物可不够时间拍照
- * _如何解决？_
...
2. 通用物体检测时，主体过多导致识别不全或只能识别一个主体
![r](r.jpg "r.jpg")
![通用识别](通用识别.png "通用识别.png")
返回结果可看到，所需要识别的图中有8个人，最终结果返回才识别到1个人.除此之外，它所选择识别的那1个人是系统自动选择识别的。

## 二、原型

### 原型1.交互及界面设计

1. 首页：用户在此页面选择2-拍照上传或5-相册上传。

![首页](首页.png "首页.png")

2. 拍照页面：用户选择拍照上传后跳转此页面，首先提醒用户对准目标物体拍照。

![拍照识别页](拍照识别页.png "拍照识别页.png")

3. 拍照上传页：用户完成拍照动作后需要点击提交按钮。

![拍照上传页](拍照上传页.png "拍照上传页.png")

4. 相册上传：用户在首页选择相册上传后会弹出弹窗询问用户是否能访问本地相册，若用户点击“是”则跳转5-上传图片选择页；若用户点击“否”则返回首页。

![相册上传](相册上传.png "相册上传.png")

5. 上传图片选择页：此页面显示的是本地相册里的图片，用户选中想要识别的图片后需要点击提交。

![上传图片选择](上传图片选择.png "上传图片选择.png")

6. 返回结果-识别失败页1：这是图片识别失败返回的页面，提示用户可以用以下方式再次尝试。

![识别失败页-1](识别失败页-1.png "识别失败页-1.png")

4. 返回结果-识别失败页2：若再次识别失败，则返回以下文字表示抱歉。

![识别失败页-2](识别失败页-2.png "识别失败页-2.png")

6. 返回结果-识别成功页：若是识别成功会跳转至此页面，内含百科名称及信息。

![识别成功页](识别成功页.png "识别成功页.png")
...
- 图像识别在人工智能和机器学习中都处于完善阶段，但未来发展前景应会逐步提高。

| 类别     | 选用公司 | 竞争者           |
| -------- | -------- | ---------------- | 
| 图像识别 | 百度   | 阿里云 |  

- 图像识别需要的数据量很大，目前公司挺难实现收集各类数据。
